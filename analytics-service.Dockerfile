...
COPY ./service/analytics-service/src ./service/analytics-service/src

RUN mvn clean package --projects :common,:analytics-service -DskipTests
...
ENV APP_PORT=8080
COPY --from=build /home/app/service/analytics-service/target/analytics-service*.jar /usr/local/lib/analytics-service.jar
EXPOSE ${APP_PORT}
ENTRYPOINT ["java","-jar","/usr/local/lib/analytics-service.jar"]