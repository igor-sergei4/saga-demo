...
COPY ./service/execution-service/src ./service/execution-service/src

RUN mvn clean package --projects :common,:execution-service -DskipTests
...
ENV APP_PORT=8080
COPY --from=build /home/app/service/execution-service/target/execution-service*.jar /usr/local/lib/execution-service.jar
EXPOSE ${APP_PORT}
ENTRYPOINT ["java","-jar","/usr/local/lib/execution-service.jar"]