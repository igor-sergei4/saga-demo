...
COPY ./service/order-service/src ./service/order-service/src

RUN mvn clean package --projects :common,:order-service -DskipTests
...
ENV APP_PORT=8080
COPY --from=build /home/app/service/order-service/target/order-service*.jar /usr/local/lib/order-service.jar
EXPOSE ${APP_PORT}
ENTRYPOINT ["java","-jar","/usr/local/lib/order-service.jar"]